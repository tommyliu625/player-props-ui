...
import { ABBREVIATION_TO_TEAM_ID, LOCAL_HOST, PLAYER_PROPS_AGAINST_OPPONENT_PRIZE_PICKS_API } from '../../../constants/propConstants';

import axios from 'axios';

const PrizePicksSingleStat = ({ projections }) => {
  ...
  useEffect(() => {
    async function fetchStatsAgainstOpponent() {      
      if (open && !modalInfo.statType.includes('Combo')) {
        ...
        let response = await axios.post(LOCAL_HOST + PLAYER_PROPS_AGAINST_OPPONENT_PRIZE_PICKS_API,
          request,
  ...
  return (
    <div className='player-projection-container'>
      {projections.length > 0 && projections.map(({playerId, projId, statType, fullName, lineScore, overStreak,
        ...
        <Card key={projId} className='player-projection-card'>
          ...
          <Button onClick={() => {
            ...
            handleModal(true, { lineHistory: reverseLineHistory, fullName, statType, lineScore, opposingTeam, projId, playerId })
...
export default PrizePicksSingleStat;